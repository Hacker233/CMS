...
Vue.use({
    install (Vue, options) {
        ...
        Vue.prototype.$api = global.apiNode;
    }
});
...
$.showSider=function (vue,obj,title,width="30%",attr) {
    var ele=document.createElement("div");
    vue.$el.appendChild(ele);
    var self = vue;
    var Child = Vue.extend(require("component/sider.vue"));
    var child = new Child({
        el: ele,
        parent: self,
        propsData:{
...
$.basePath=function () {
    if(window.debug)
    {
        ...
        var index=path.indexOf("/console/");
        return path.substring(0,index+"/console/".length)
    }
    else
    {
        var path=location.href;
        var index=path.indexOf("/content/");
        return path.substring(0,index+"/content/".length)
    }
}
...
$.getProjectStore=function (path) {
    if(!window.projectStore)
    {
        return null;
    }
    let obj=window.projectStore[sessionStorage.getItem("teamId")?sessionStorage.getItem("teamId"):""];
    if(obj)
    {
        return obj._modulesNamespaceMap[path?path:""].context
    }
    else
    {
        return null;
...
$.pluginCall=function(handler,vue){
    handler.call(vue,require("common/js/config"),$,require("common/js/net"),require("common/js/helper"),require("common/js/db").db);
}
